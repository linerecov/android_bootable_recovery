...
void ScreenRecoveryUI::draw_menu_and_text_buffer_locked(
  ...
  if (menu_) {
    ...
    auto& logo = fastbootd_logo_enabled_ ? fastbootd_logo_ : aqua_logo_;
    if (logo && back_icon_) {
...
bool ScreenRecoveryUI::Init(const std::string& locale) {
  ...
  aqua_logo_ = LoadBitmap("logo_image");
...
std::unique_ptr<Menu> ScreenRecoveryUI::CreateMenu(const std::vector<std::string>& text_headers,
  ...
  int menu_rows = (ScreenHeight() - margin_height_*2 - gr_get_height(aqua_logo_.get()))
...
int ScreenRecoveryUI::SelectMenu(const Point& point) {
  ...
  if (menu_) {
    if (!menu_->IsMain()) {
      // Back arrow hitbox
      const static int logo_width = gr_get_width(aqua_logo_.get());
      const static int logo_height = gr_get_height(aqua_logo_.get());
